...
function renderProdutos() {
    ...
    todosProdutos.forEach(p => {
        ...
        html += `<div class="produto-card ${statusClass}">
            ...
            <div class="produto-validade">📅 Vence: ${dataVenc}</div>
            ...
            <div class="produto-actions">
                <button class="btn-icon edit" onclick="abrirModalEditarProduto(${p.id},'${escapeHtml(p.nome)}',${p.loja},${p.deposito},'${p.vencimento}')">✏️</button>
...
function abrirModalEditarProduto(id, nome, loja, deposito, vencimento) {
...
function gerarMensagemWhatsApp() {
    ...
    mensagem += `*ESTOQUE TOTAL*\n`;
    mensagem += `🏪 Loja: ${totalLoja.toLocaleString('pt-BR')} unidades\n`;
    mensagem += `🏢 Depósito: ${totalDeposito.toLocaleString('pt-BR')} unidades\n`;
    mensagem += `📦 Total: ${totalGeral.toLocaleString('pt-BR')} unidades\n\n`;
    
    mensagem += `*PRODUTOS DETALHADOS:*\n`;
    
    todosProdutos.forEach(p => {
        const total = p.loja + p.deposito;
        const statusEmoji = p.status === 'vencido' ? '❌' : p.status === 'aviso' ? '⚠️' : '✅';
        const dataVenc = new Date(p.vencimento).toLocaleDateString('pt-BR');
        mensagem += `\n${statusEmoji} *${escapeHtml(p.nome).toUpperCase()}* (Vence: ${dataVenc})\n`;
        mensagem += `🏪 Loja: ${p.loja} | 🏢 Depósito: ${p.deposito} | 📦 Total: ${total}\n`;
    });
